...
**Address, not name+group.** Every agent has a 5-segment address `user/host/runtime/project/session`. `user` is stamped by muster-api from `x-muster-agent`'s caller (the API key); the shim supplies `host/runtime/project/session` via the `x-muster-agent` header (`naming.derive_address`). `to` (chat) and `project`/`scope` filters accept any contiguous, unique slice of an address; ambiguous references get back `candidates` to retry with a longer slice.
